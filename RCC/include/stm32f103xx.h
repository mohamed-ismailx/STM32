...
typedef struct 
{
	CR_Access CR;          	  //Clock Control Register By Base Address
	CFGR_Access CFGR;		  //Clock Configuration Register
    ...
    u32 AHB2ENR;              //APB2 Peripheral Clock Enable Register
    u32 AHB1ENR;              //APB1 Peripheral Clock Enable Register
    u32 BDCR;                 //Backup Domain Control Register
    u32 CSR;                  //Control/Status Register

}RCC_Registers;

#define RCC ((volatile RCC_Registers *) RCC_Base_Address)       //Pointer To Structure To Access RCC Registers By Base Address
...
typedef struct
{
	CRL_Access CRL;		   	   //Port Configuration Register Low For First 8 Bit (0 --> 7)
	CRH_Access CRH;		       //Port Configuration Register HIGH For Last 8 Bit (8 --> 15)
	IDR_Access IDR;		       //Port Input Data Register For Read Input Data On Pin
	ODR_Access ODR;		       //Port Output Data Register For Configuration The Output State (High/Low) || --> For Pull Up (1) High (0) Low || --> For Open Drain (1) Floating (0) Low
	u32 BSRR;				   //Port Bit Set/Reset Register For Configuration ODR Atomic Access By High State (1)
	u32 BRR;				   //Port Bit Reset Register For Configuration ODR Atomic Access By Low State (0)
	LCKR_Access LCKR;		   //Port Configuration Lock Register For Lock Bit Configuration In RunTime

}GPIO_Registers;

#define GPIOA_Base_Address     ((volatile GPIO_Registers*)0x40011000)    //Pointer To Structure To Access GPIO Registers By Base Address Of GPIOA
#define GPIOB_Base_Address     ((volatile GPIO_Registers*)0x40010C00)	 //Pointer To Structure To Access GPIO Registers By Base Address Of GPIOB
#define GPIOC_Base_Address     ((volatile GPIO_Registers*)0x40010800)    //Pointer To Structure To Access GPIO Registers By Base Address Of GPIOC
